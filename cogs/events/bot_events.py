...
class BotEvents(commands.Cog):
    ...
    async def on_member_join(self, member):
        try:
            ...
            channel = self.bot.get_channel(int(config['channel_text']['introduction_channel_text']))
            msg = f'Welcome {member.mention} to the Universe 7 training camp! '
            await channel.send(msg)
            new_mem_msg = f'{member.name} joined {guild.name} on {datetime.datetime.now()}'
            send_log(new_mem_msg, log_location)
            await member.add_roles(role, reason=f"User joined the guild {guild.name}")
            await channel.send(content="You are now recognized by the Omni-King as a mortal wishing to social, grow,"
                                       " and most importantly train in his realm! "
                                       "Enjoy your stay, I will be watching over you")
            given_role_msg = f"{member.name} given Mortal role in {guild.name} on {datetime.datetime.now()}"
